...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        # there are no legal actions, at the terminal state, return a value of 0.0
        if len(actions) == 0:
          return 0.0
        values = util.Counter()
        maxQ = float("-inf")
    ...
    def computeActionFromQValues(self, state):
        ...
        # you only access Q values by calling getQValue
        actions = self.getLegalActions(state)
        # there are no legal actions, at the terminal state, return a value of None
        if len(actions) == 0:
          return None
        maxQ = float("-inf")
        for action in actions:
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        alpha = self.alpha
        gamma = self.discount
        V = (1 - alpha) * self.getQValue(state, action)
        sample = reward + gamma*self.getValue(nextState)
        V = V + alpha*sample
        self.Q_values[(state,action)] = V
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        for feat in feature:
          self.weights[feat] += self.alpha * (reward + self.discount * max_q_next - action_val) * feature[feat]
